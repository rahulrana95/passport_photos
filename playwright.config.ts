...
/**
 * A baseline may only be written by the browser CI will compare it with.
 *
 * This is not caution, it is a bug that has already happened. Baselines
 * generated against a different Chromium build than the one Playwright pins
 * look perfect locally and fail on CI, and the failure is a screenshot diff —
 * which reads as "somebody changed the design", not as "the wrong browser
 * wrote this file". It cost a red main and three rounds of misdiagnosis,
 * during which the real signal was repeatedly reverted as noise.
 *
 * Running the suite against another Chromium is still fine, and still useful
 * in a sandbox. Only WRITING baselines with one is refused.
 */
if (chromiumExecutable !== undefined && process.argv.includes('--update-snapshots')) {
  throw new Error(
    'Refusing to write screenshot baselines with PLAYWRIGHT_CHROMIUM_EXECUTABLE set: ' +
      'they would be generated by a different Chromium than CI compares against. ' +
      'Unset it and let Playwright use the revision it pins.',
  );
}
